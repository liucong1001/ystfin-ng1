...
app.config(["$routeProvider",function($routeProvider){
    $routeProvider.when("/pay/:id/exchangepay",{
        controller:"exchangePayController",
        template:require("./html/order/exchangePay.html")
    })
}])
app.controller("exchangePayController",["$scope","Order","$routeParams","$http","$icard","$filter","md5",function ($scope,Order,$routeParams,$http,$icard,$filter,md5) {
    ...
    $scope.pay = function () {
        ...
        $icard.getPassword().then(function (pwd) {
            ...
            $http.post("/exchange/pay", {
                ...
                $scope.cardMessage = "扣款成功\n当前余额:" + $filter("currency")(result.data.icCard.balance / 100);
                $icard.playVoice(6);
                //scanCard()
            }, function (result) {
