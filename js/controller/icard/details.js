...
app.controller("iCardDetailsController",["$scope","$routeParams","AccountRecords",function ($scope,$routeParams,AccountRecords) {
    ...
    $scope.tableColumns = [
        ...
        {title:"金额",template:"{{row.amount | currency:'￥'}}",thClass:"text-center",tdClass:"text-center", width:20},
        {title:"处理时间",template:"{{row.createTime | date:'yyyy-MM-dd'}}",thClass:"text-center",tdClass:"text-center", width:20},
        {title:"类型",template:"<ng-convert code='icard_type' value='{{row.type}}' ></ng-convert>",width:20,thClass:"text-left",tdClass:"text-left"},
        {title:"订单详情",template:"<a href='/ng#/pay/order/query/{{row.orderNo}}'>查看</a>",width:20,thClass:"text-left",tdClass:"text-left"}
    ]
