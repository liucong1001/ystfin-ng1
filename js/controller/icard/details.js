...
app.controller("iCardDetailsController",["$scope","$routeParams","AccountRecords",function ($scope,$routeParams,AccountRecords) {
    ...
    $scope.tableColumns = [
        ...
        {title:"订单详情",template:"<a href='/ng#/pay/order/query?icardNo={{row.orderNo}}&accountId={{row.account.id}}'>{{row.orderNo ? '查看':''}}</a>",width:20,thClass:"text-left",tdClass:"text-left"}
    ]
