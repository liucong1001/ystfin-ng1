...
app.controller("accountController",["$scope","$routeParams","$http","$icard","$location",function ($scope,$routeParams,$http,$icard,$location) {
    ...
    $scope.tableColumns = [
        {title:"卡号",template:"{{row.cardNo}}", width:12,thClass:"text-center",tdClass:"text-center"},
        ...
        {title:"操作",template:"<a href='' ng-if='row.status != 02' ng-click='instance.cardLoss(row.dealers.id,row.cardNo)'>挂失</a>&nbsp;&nbsp;<a href='' ng-click='instance.removeBind(row.cardNo)'>注销解绑</a> ",width:15,thClass:"text-center",tdClass:"text-center"}
    ]
    ...
    $scope.ngTable={
        ...
        cardLoss:function (delars,cardNo) {
            if(confirm("是否确定【挂失】此卡片？")){
                $http.post("/icard/cardLoss",{delars:delars,cardNo:cardNo}).then(function () {
                    $scope.ngTable.reload();
                })
            }
        },
        removeBind:function (cardNo) {
            if(confirm("是否确定【注销解绑】此卡片？")){
                $http.post("/icard/removeBind",{cardNo:cardNo}).then(function () {
                    $scope.ngTable.reload();
                })
            }
        }
